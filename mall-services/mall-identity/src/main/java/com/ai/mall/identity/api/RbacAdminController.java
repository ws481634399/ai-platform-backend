...
import com.ai.mall.identity.rbac.RbacAdministrationService;
...
public class RbacAdminController {
    private final RbacAdministrationService service;
    public RbacAdminController(RbacAdministrationService service) { this.service = service; }
    ...
    public UnifyResult<Void> createMenu(@Valid @RequestBody MenuNode menu) {
        service.createMenu(actor(), menu); return UnifyResult.ok();
    }
    ...
    public UnifyResult<Void> updateMenu(@PathVariable long id, @Valid @RequestBody MenuUpdate request) {
        service.updateMenu(actor(), new com.ai.mall.identity.persistence.RbacCommandMapper.MenuRow(id, request.parentId(), request.name(), request.type(), request.path(), request.componentKey(), request.permissionCode(), request.sortOrder(), request.visible(), request.status()));
        return UnifyResult.ok();
    }
